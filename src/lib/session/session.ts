...
import { isAllowedOperatorEmail } from "./allowed-email";
...
export async function getSessionOperator(): Promise<Operator | null> {
  ...
  const email =
    ...
    "";
  // Second layer behind the proxy gate: never link or create an operator for
  // an account outside the allowed domain.
